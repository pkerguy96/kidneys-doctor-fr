...
const ReferralPatient = ({ setLoading }) => {
    ...
    const { data: referal = {}, isLoading } = getGlobal({}, CACHE_KEY_PatientReferral, PatientsReferralClient, { staleTime: 360000 });
    ...
    useEffect(() => {
        ...
        if (JSON.stringify(data.labels) !== JSON.stringify(chartData.labels) ||
    ...
    return (_jsx(Doughnut, { data: data, options: { layout: { padding: { bottom: 2 } } } }));
};
